//! Rainbow smoke test for the hub75 crate.
//!
//! Solid R / G / B / W fills (3 s each), then a scrolling diagonal
//! rainbow forever. Validates the full pipeline: `frame_mut` borrow,
//! pixel writes, `commit`, on-screen output.
...
async fn main(spawner: Spawner) {
    ...
    defmt::info!("hub75 rainbow example running");

    show_solid_fills(&mut panel).await;
    show_scrolling_rainbow(&mut panel).await;
}

async fn show_solid_fills(panel: &mut Dp3364sPanel) {
    let palette = [
        Rgb::new(255, 0, 0),
        Rgb::new(0, 255, 0),
        Rgb::new(0, 0, 255),
...
async fn show_scrolling_rainbow(panel: &mut Dp3364sPanel) -> ! {
    let mut offset: u8 = 0;
    loop {
        let frame = panel.frame_mut().await;
