...
Firmware for the panel that `sysmon` drives. Thin shell over [`usb-firmware-lib`](../../usb/firmware-lib/) — the bulk of the firmware logic (USB descriptor, frame protocol, button polling) lives in the library; this crate only supplies the sysmon-specific bits:
