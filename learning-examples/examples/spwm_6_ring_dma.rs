//! # DP3364S S-PWM — Step 6: Ring-mode DMA scan
//!
//! Sixth in the SPWM progression. Builds on `spwm_5_unified_sm.rs`.
//!
//! **What this step adds:** RP2350 DMA ring mode for continuous scan
//! without CPU intervention, eliminating micro-gaps between scan passes.
//!
//! The scan DMA (ch1) uses ring mode with RING_SIZE=7 to wrap the read
//! address every 128 bytes (32 words × 4 bytes), looping through the
//! scan buffer indefinitely. The DMA hardware handles address wrapping
//! in silicon — no CPU touch needed between scan passes.
//!
//! Data DMA (ch0) uses the HAL's single_buffer as before. Only one
//! channel runs at a time: ch1 is stopped before ch0 sends frame data,
//! then ch1 is restarted.
//!
//! ```text
...
//!
//! ```sh
//! cargo run --release --example spwm_6_ring_dma
//! ```
