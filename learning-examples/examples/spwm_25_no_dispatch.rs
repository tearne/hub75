...
fn main() -> ! {
    ...
    // Drop LAT; wrap fires AFTER this instruction (back to data_cmd)
    ...
    a.jmp_with_side_set(pio::JmpCondition::Always, &mut scan_entry, 0);

    let prog = a.assemble_with_wrap(wrap_source, data_cmd);
    let installed = pio0.install(&prog).unwrap();

    // Entry-point absolute PC addresses.
    // data_cmd is the first emitted instruction → at installed.offset().
    // scan_entry is at offset + N where N = data-path instruction count.
    //
    // Data-path instruction count (must stay in sync with assembly above):
    //   1. OUT X 15
    //   2. OUT Y 16
    //   3. OUT NULL 1
    //   4. OUT PINS 6       (pre_loop start)
    //   5. OUT NULL 2
    //   6. JMP X-- pre_loop
    //   7. SET PINS 0b01    (raise LAT)
    //   8. OUT PINS 6       (lat_loop start)
    //   9. OUT NULL 2
    //  10. JMP Y-- lat_loop
    //  11. SET PINS 0b00    (drop LAT) — wrap_source is AFTER this
    const SCAN_ENTRY_LOCAL_OFFSET: u32 = 11;
    let prog_offset = installed.offset() as u32;
    let data_cmd_pc = prog_offset;
    let scan_entry_pc = prog_offset + SCAN_ENTRY_LOCAL_OFFSET;
    defmt::info!(
        "spwm_25 PIO offsets: data_cmd={=u32} scan_entry={=u32}",
    ...
    defmt::info!("spwm_25: unified program, no dispatch, forced-JMP entry points");
