//! # DP3364S S-PWM — Step 28: PINCTRL swap at phase boundary
//!
//! ## Purpose
//!
//! Thread B review (2026-04-24) identified one untested firmware-side
//! variable: **PINCTRL register contents across the phase boundary**.
//! spwm_17/22 (echo-free) swap PINCTRL at each phase transition —
//! OUT_COUNT, SET_BASE/SET_COUNT, SIDESET_COUNT all change. Every
//! unified-program attempt (spwm_23–27) holds PINCTRL constant
//! through the transition. spwm_24/25/27 specifically echo despite
//! matching the scan-phase instruction encoding to spwm_22.
//!
//! This test adds PINCTRL swap to spwm_27's architecture (unified
//! program + runtime EXECCTRL swap) and changes two PINCTRL fields
//! that can vary under the ss=1 unified constraint:
//!   - OUT_COUNT: 6 (data) ↔ 11 (scan)
//!   - SET_BASE/SET_COUNT: 12/2 (data, LAT+OE) ↔ 13/1 (scan, OE only)
//! SIDESET_COUNT stays at 1 (ss=1 unified; can't replicate spwm_22's
//! 2↔1 swap without spwm_26's reinterpretation bug).
//!
//! Scan path's `set PINS 0b10` (OE on under SET_BASE=12/COUNT=2) is
//! rewritten to `set PINS 0b01` (OE on under SET_BASE=13/COUNT=1).
//! Data path unchanged: `set PINS 0b01` = LAT on under SET_BASE=12/2.
//!
//! ## Outcomes
//!
//! - **Echo-free + display correct** → PINCTRL swap is load-bearing.
//!   Mechanism is runtime PINCTRL change at phase boundary.
//! - **Echo present** → PINCTRL swap is NOT the mechanism either.
//!   All runtime-register-swap hypotheses exhausted; firmware thread
//!   definitively closed.
//! - **Garbled display** → SET-instruction rewrite bug or PINCTRL
//!   arithmetic error; debug.
//!
//! ```sh
//! cargo run --release --example spwm_28_pinctrl_swap
//! ```
//!
...
fn main() -> ! {
    ...
    defmt::info!(
        "spwm_28 PIO: data_cmd={=u32} scan_entry={=u32}  data_wrap_top={=u32}  scan_wrap_top={=u32}",
    ...
    defmt::info!(
        "spwm_28 EXECCTRL: base=0x{=u32:08x} data=0x{=u32:08x} scan=0x{=u32:08x}",
    ...
    //   [4:0]   OUT_BASE
    //
    // Data phase (write RGB only via `out PINS, 6`; LAT via SET at
    // pin 12 with bit 0):
    //   OUT_BASE=0, OUT_COUNT=6, SIDESET_BASE=11, SIDESET_COUNT=1,
    //   SET_BASE=12, SET_COUNT=2, IN_BASE=0
    //
    // Scan phase (write RGB+ADDR via `out PINS, 11`; OE via SET at
    // pin 13 with bit 0, after the 0b10→0b01 instruction rewrite):
    //   OUT_BASE=0, OUT_COUNT=11, SIDESET_BASE=11, SIDESET_COUNT=1,
    //   SET_BASE=13, SET_COUNT=1, IN_BASE=0
    ...
    defmt::info!(
        "spwm_28 PINCTRL:  base=0x{=u32:08x} data=0x{=u32:08x} scan=0x{=u32:08x}",
    ...
    defmt::info!("spwm_28: unified program + PINCTRL + EXECCTRL swap at phase boundary");
