//! # DP3364S S-PWM — Step 27: EXECCTRL-only persistent swap
//!
//! ## STATUS (2026-04-24): WRAP_TOP off-by-one fix applied, awaiting flash
//!
//! Prior symptom (2026-04-23): scan DMA hangs in flush iter 0 after
//! `swap_to_scan`. Root cause identified by static analysis of
//! pio-0.2.1/src/lib.rs:574 — `assemble_with_wrap` does `source -= 1`
//! internally because `bind` labels the NEXT instruction slot. This
//! file's runtime EXECCTRL computation used the raw label values as
//! WRAP_TOP, yielding WRAP_TOP=27 for scan. PC 27 is past the end of
//! the installed program, so after `out NULL, 5` at PC 26 the SM fell
//! through into garbage (likely 0x0000 = JMP 0), landing back at
//! data_cmd and consuming the remaining scan words as data-headers
//! before stalling on empty FIFO.
//!
//! Fix: constants renamed DATA_WRAP_TOP_LOCAL=10, SCAN_WRAP_TOP_LOCAL=26
//! to hold hardware WRAP_TOP directly. Added sanity log comparing
//! base_execctrl (what PIOBuilder wrote for data wrap) vs computed
//! data_execctrl — they must match.
//!
...
fn main() -> ! {
    ...
    defmt::info!(
        "spwm_27 PIO: data_cmd={=u32} scan_entry={=u32}  data_wrap_top={=u32}  scan_wrap_top={=u32}",
    ...
    defmt::info!(
        "spwm_27 EXECCTRL: base=0x{=u32:08x} data=0x{=u32:08x} scan=0x{=u32:08x}",
        base_execctrl, data_execctrl, scan_execctrl,
    );

    // Phase-swap closures — write EXECCTRL + force JMP. PINCTRL
    // stays put at side_set_count=1.
    ...
    defmt::info!("spwm_27: unified program + persistent EXECCTRL wrap swap");
