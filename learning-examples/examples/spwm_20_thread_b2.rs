//! # DP3364S S-PWM — Step 20: Thread B experiment 2 (swap-component isolation)
//!
//! ## Purpose
//!
//! Identify which specific register-write inside spwm_17's
//! `swap_sm0_program` is what the DP3364S treats as a pipeline reset
//! (→ kills the scan_line-31 → scan_line-0 SRAM echo). If one
//! component alone reproduces the fix, the full program-swap collapses
//! to that single write.
//!
//! ## Status coming in
//!
//! - **Experiment 1 (CLK-idle pause, spwm_19):** FALSIFIED 2026-04-23.
//!   Sweeping `BOUNDARY_PAUSE_CYCLES` from 0 to 10_000 cycles of CPU
//!   delay (with PIO stalled → genuine CLK idle) never reduced the
//!   echo. Combined with spwm_12's earlier 100_000-cycle null, CLK
//!   silence alone is not the mechanism.
//! - **disable/enable bracketing:** already in spwm_12 (lines
//!   `disable_sm(SM0_EN)` after wait_txstall, `enable_sm(SM0_EN)`
//!   before next phase) and spwm_12 echoes. Meanwhile `swap_sm0_program`
//!   in spwm_17 does **not** disable the SM — it writes to a running
//!   (stalled) SM. So run-state toggling is doubly ruled out.
//!
//! Remaining candidates, each a register write that swap_sm0_program
//! performs while the SM is stalled:
//!
//! 1. `SM0_PINCTRL` — side-set / out / set pin mapping
//! 2. `SM0_EXECCTRL` — wrap-source / wrap-target
//! 3. `SM0_INSTR` — forced JMP (PC reset)
//!
//! ## Experiment
//!
//! Architecture is spwm_12 verbatim (unified PIO program with type-bit
//! dispatch). At the data→scan boundary, after `wait_txstall` and
//! `disable_sm`, apply ONE of the three register touches in isolation,
//! save-and-restore pattern so the SM resumes in the same logical
//! state as baseline. Constant `ISOLATION_VARIANT` selects which:
//!
//! ### Phase A — variant applied while SM disabled (flawed, 2026-04-23)
//!
//! | value | variant           | observation   |
//! | ----- | ----------------- | ------------- |
//! | 0     | none (baseline)   | echo present  |
//! | 1     | PINCTRL rewrite   | echo present  |
//! | 2     | EXECCTRL rewrite  | echo present  |
//! | 3     | forced JMP        | echo present  |
//!
//! Phase A is inconclusive because `SM0_INSTR` writes to a disabled SM
//! may be silently discarded (no pipeline cycle to inject into), and
//! the other register writes' side effects can depend on SM liveness.
//! Revised flow keeps the SM enabled-and-stalled throughout, matching
//! spwm_17's `swap_sm0_program` semantics.
//!
//! ### Phase B — variant applied while SM enabled + stalled
//!
//! | value | variant           | observation   |
//! | ----- | ----------------- | ------------- |
//! | 0     | none (baseline)   | echo present  |
...
//! | 3     | forced JMP        |               |
//!
//! Test order: baseline (0) to confirm echo present, then 1 → 2 → 3.
//! If any kills the echo, that names the mechanism. If none does, the
//! mechanism may require a combination, or may depend on the specific
//! value written (the save-and-restore pattern tests "register touched
//! at all", not "register given a particular value").
//!
//! Prior ranking by plausibility: PINCTRL > forced JMP > EXECCTRL.
//! PINCTRL changes what pins the SM drives — a transient rewrite plus
//! restore could plausibly produce a visible edge pattern at the chip.
//! EXECCTRL and INSTR are purely PIO-internal state until the SM
//! actually executes, which it doesn't while stalled.
//!
//! ```sh
//! cargo run --release --example spwm_20_thread_b2
//! ```
//!
//! ---
//!
//! # Original spwm_12 header follows (architecture reference + E21-E25 log)
//!
...
// Thread B experiment 2: which single register-touch from spwm_17's
// swap_sm0_program kills the echo?
//   0 = none (baseline — should echo, same as spwm_12)
//   1 = SM0_PINCTRL rewrite (save → write alternate → restore)
//   2 = SM0_EXECCTRL rewrite (save → write alternate → restore)
//   3 = forced JMP via SM0_INSTR (JMP to program start)
// All applied while SM0 is disabled + stalled (bracket of wait_txstall
// + disable_sm). Save-and-restore so the SM resumes in baseline state.
const ISOLATION_VARIANT: u32 = 3;
...
fn main() -> ! {
    ...
    let prog_start: u32 = installed.offset() as u32;
    ...
    defmt::info!("spwm_20 Thread B exp2: ISOLATION_VARIANT = {=u32} (0=none 1=PINCTRL 2=EXECCTRL 3=JMP)", ISOLATION_VARIANT);
    ...
    loop {
        ...
        // Thread B exp 2 (revised 2026-04-23): apply the selected swap-
        // component while SM is ENABLED + stalled on autopull (FIFO empty).
        // Earlier revision disabled the SM first; SM0_INSTR writes to a
        // disabled SM may be silently discarded, so variant 3 wasn't
        // truly executing. Match spwm_17 where swap_sm0_program writes
        // to a stalled-but-live SM.
        match ISOLATION_VARIANT {
            1 => unsafe {
                // PINCTRL rewrite: save → zero → restore.
                let saved = (SM0_PINCTRL as *const u32).read_volatile();
                (SM0_PINCTRL as *mut u32).write_volatile(0);
                (SM0_PINCTRL as *mut u32).write_volatile(saved);
            },
            2 => unsafe {
                // EXECCTRL rewrite: save → flip STATUS_SEL bit → restore.
                // (STATUS_SEL is a low-impact field; toggling it produces
                // a register-write transaction without breaking the SM.)
                let saved = (SM0_EXECCTRL as *const u32).read_volatile();
                (SM0_EXECCTRL as *mut u32).write_volatile(saved ^ (1 << 4));
                (SM0_EXECCTRL as *mut u32).write_volatile(saved);
            },
            3 => unsafe {
                // Forced JMP to program start — matches spwm_17's swap.
                let jmp_always = prog_start & 0x1F;
