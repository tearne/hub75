//! # DP3364S S-PWM — Step 19: Thread B experiment 1 (CLK-idle pause sweep)
//!
//! ## Purpose
//!
//! Formally close the CLK-idle-pause hypothesis for the spwm_17 program-
//! swap echo fix. Hypothesis: the chip treats any sufficiently long
//! period of CLK silence at the data→scan boundary as a pipeline reset,
//! and that is what kills the scan_line-31 → scan_line-0 SRAM echo. If
//! true, spwm_17's whole program-swap machinery could collapse to a
//! single wait.
//!
//! ## Status coming in
//!
//! spwm_12's `INTER_DMA_DELAY_CYCLES` was previously swept to 100_000
//! cycles (~670 µs) with no echo reduction. So the hypothesis is very
//! likely already dead; this file exists to run an explicit sweep at
//! documented values so the null is cleanly on record for project
//! memory.
//!
//! ## Experiment
//!
//! Architecture is spwm_12 verbatim (unified PIO program with type-bit
//! dispatch, split DMA with wait_txstall between phases, per-cycle
//! post-scan DMAs). Only change: one knob `BOUNDARY_PAUSE_CYCLES` that
//! inserts a `cortex_m::asm::delay` at the data→scan boundary, after
//! wait_txstall (so PIO is confirmed stalled → CLK genuinely idle for
//! the full pause duration).
//!
//! Sweep values (edit, flash, observe E25 pattern, record):
//!
//! | value   | ≈ time @150 MHz | echo on row 0 / row 32 |
//! | ------- | --------------- | ---------------------- |
//! | 0       | 0               | echo present (baseline) |
//! | 50      | ~333 ns         | echo present           |
//! | 100     | ~667 ns         | echo present           |
//! | 500     | ~3.3 µs         | echo present           |
//! | 1_000   | ~6.7 µs         | echo present           |
//! | 5_000   | ~33 µs          | echo present           |
//! | 10_000  | ~67 µs          | echo present           |
//!
//! ## Result (2026-04-23) — CLK-idle-only hypothesis FALSIFIED
//!
//! Echo present at every sweep value from 0 to 10_000 cycles
//! (~0 to ~67 µs of genuine CLK silence at the data→scan boundary).
//! Combined with the earlier spwm_12 100_000-cycle (~670 µs) null,
//! CLK idle time alone does not trigger whatever pipeline reset the
//! spwm_17 program-swap causes on the DP3364S.
//!
//! What this leaves for the echo-killing mechanism in spwm_17:
//!   - `SM0_EXECCTRL` rewrite (wrap-source / wrap-target change)
//!   - `SM0_PINCTRL` rewrite (side-set / out / set pin mapping)
//!   - Forced `SM0_INSTR` JMP to new program entry
//!   - The `disable_sm` that brackets the swap (SM run-state toggle)
//!
//! None of these is pure CLK idling — each changes either the pin
//! mapping observed at the chip or the SM's instruction stream.
//!
//! ## Next: experiment 2 (or a refocused Thread B)
//!
//! Build spwm_20 that applies one of the above in isolation while
//! keeping spwm_12's unified-program architecture, to identify which
//! single component of `swap_sm0_program` kills the echo.
//!
//! ```sh
//! cargo run --release --example spwm_19_thread_b
//! ```
//!
//! ---
//!
...
// Thread B experiment 1: CLK-idle pause at the data→scan boundary.
// Inserted after wait_txstall so PIO is confirmed stalled (CLK line
// genuinely idle) for the full duration. Sweep values in the file
// header table. Each step = one cortex-m cycle ≈ 6.67 ns at 150 MHz.
const BOUNDARY_PAUSE_CYCLES: u32 = 10_000;
...
fn main() -> ! {
    ...
    defmt::info!("spwm_19 Thread B exp1: BOUNDARY_PAUSE_CYCLES = {=u32}", BOUNDARY_PAUSE_CYCLES);
    ...
    loop {
        ...
        if BOUNDARY_PAUSE_CYCLES > 0 {
            cortex_m::asm::delay(BOUNDARY_PAUSE_CYCLES);
        }
