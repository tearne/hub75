//! # DP3364S S-PWM — Step 30: OUT_COUNT-only PINCTRL swap (narrowing)
//!
//! ## Purpose
//!
//! spwm_28 (both fields swapped) was echo-free. spwm_29 (SET swap
//! only, OUT_COUNT=11 in both phases) had the echo back. That points
//! to OUT_COUNT as the load-bearing PINCTRL field, against my prior
//! prediction. This variant tests the complement: **OUT_COUNT swap
//! only**, SET_BASE/SET_COUNT held at 12/2 for both phases (same as
//! spwm_27). Scan's SET instruction stays `set PINS 0b10` (OE on
//! under SET_BASE=12, COUNT=2 — pin 13 high).
//!
//! Prediction: echo-free. Would confirm OUT_COUNT swap alone is
//! sufficient and the SET swap in spwm_28 was incidental.
//!
//! ## Mechanism hypothesis
//!
//! When `out PINS, N` executes with bit_count < OUT_COUNT, the PIO
//! likely zero-fills pins OUT_BASE+N through OUT_BASE+OUT_COUNT-1.
//! With OUT_COUNT=11 during data (spwm_27/29), every `out PINS, 6`
//! in pre_loop and lat_loop forces pins 6-10 (ADDR) to 0. With
//! OUT_COUNT=6 during data (spwm_28/30), pins 6-10 retain their
//! end-of-scan value (scan_line 31 = 0b11111). That difference —
//! ADDR hammered to 0 ~130k times per frame vs held at 31 — is a
//! real electrical delta the DP3364S may respond to.
//!
//! ## Outcomes
//!
//! - **Echo-free** → OUT_COUNT swap alone is the mechanism. SET swap
//!   is unrelated to echo suppression. Minimum viable PINCTRL swap
//!   is "change OUT_COUNT only".
//! - **Echo present** → OUT_COUNT and SET swaps interact; both
//!   needed together. More investigation required.
//!
//! Delta from spwm_28: scan_pinctrl SET fields restored to match
//! base (12/2), scan's `set PINS 0b01` reverted to `set PINS 0b10`.
//!
//! ```sh
//! cargo run --release --example spwm_30_pinctrl_out_only
//! ```
//!
...
fn main() -> ! {
    ...
    // spwm_30: scan SET_BASE=12, COUNT=2 (unchanged from base/data). OE at bit 1.
    ...
    defmt::info!(
        "spwm_30 PIO: data_cmd={=u32} scan_entry={=u32}  data_wrap_top={=u32}  scan_wrap_top={=u32}",
    ...
    defmt::info!(
        "spwm_30 EXECCTRL: base=0x{=u32:08x} data=0x{=u32:08x} scan=0x{=u32:08x}",
    ...
    //   [4:0]   OUT_BASE
    //
    // spwm_30: only OUT_COUNT differs between phases. SET and SIDESET
    // fields are identical to PIOBuilder's base (SET 12/2, SIDESET 11/1).
    //
    // Data phase: OUT_BASE=0, OUT_COUNT=6,  SIDESET=11/1, SET=12/2.
    // Scan phase: OUT_BASE=0, OUT_COUNT=11, SIDESET=11/1, SET=12/2.
    ...
    defmt::info!(
        "spwm_30 PINCTRL:  base=0x{=u32:08x} data=0x{=u32:08x} scan=0x{=u32:08x}",
    ...
    defmt::info!("spwm_30: OUT_COUNT-only PINCTRL swap (SET held at 12/2 in both phases)");
