//! # DP3364S S-PWM — Step 26: unified-program, persistent PINCTRL/EXECCTRL swap
//!
//! ## Purpose
//!
//! Test whether spwm_17/22's echo-free behaviour is caused by the
//! **persistent different register values** (EXECCTRL wrap range and
//! PINCTRL side_set_count) being held across the scan phase — rather
//! than by having two installed PIO programs at different PC offsets.
//!
//! Full Thread B prior-state review:
//!
//! | file    | single-prog | persistent EXECCTRL swap | PINCTRL swap | echo |
//! | ------- | ----------- | ------------------------ | ------------ | ---- |
//! | spwm_22 | NO (2 progs)| yes (per install)        | yes          | no   |
//! | spwm_23 | yes         | no                       | no           | echo |
//! | spwm_24 | yes         | no                       | no           | echo |
//! | spwm_25 | yes         | no                       | no           | echo |
//! | spwm_26 | yes         | **yes**                  | **yes**      | ?    |
//!
//! ## This experiment
//!
//! One unified PIO program with two entry points and two wrap
//! configurations:
//!   - data_cmd / data_wrap (wrap after lat_loop → data_cmd)
//!   - scan_entry / scan_wrap (wrap after scan body → scan_entry)
//!
//! At each phase boundary the CPU writes (three register writes):
//!   1. SM0_EXECCTRL = (data|scan)_execctrl — changes WRAP_TOP/BOTTOM
//!   2. SM0_PINCTRL  = (data|scan)_pinctrl  — changes SIDESET_COUNT
//!   3. SM0_INSTR    = JMP (data_cmd|scan_entry)
//!
//! This matches spwm_17's `swap_sm0_program` semantics exactly, but
//! with one program instead of two. side_set_count=2 during data,
//! =1 during scan; wraps are phase-specific. Scan phase ends via
//! PIO wrap (not explicit JMP), matching spwm_22 exactly.
//!
//! ## Outcomes
//!
//! - **Echo-free + display correct** → persistent register values are
//!   the mechanism. Simplification achievable: one program + 3-reg
//!   swap per phase. Approaches spwm_17's ergonomics without the
//!   two-programs complication.
//! - **Echo present** → "two installed programs at different PC
//!   offsets" is itself the load-bearing variable. This is a weird
//!   but concrete finding — firmware-only simplification dead-ends
//!   here; would need scope work to understand further.
//!
//! ## Changes from spwm_23
//!
//! 1. Remove dispatch prelude; add scan_entry label.
//! 2. Repack data_header and scan_word — no type bit.
//! 3. Data path ends with wrap (no trailing JMP dispatch).
//! 4. Scan path ends with scan_wrap_source label (relies on wrap,
//!    not explicit JMP).
//! 5. Main / flush / sync: force-JMP + EXECCTRL + PINCTRL write
//!    per phase boundary.
//!
//! ```sh
//! cargo run --release --example spwm_26_unified_swap
//! ```
//!
//! ---
//!
...
// ── Data header (no type bit — spwm_26 removes dispatch) ─────────────
...
fn main() -> ! {
    ...
    // spwm_26: 2-bit side-set assembled. At runtime PINCTRL's
    // SIDESET_COUNT is switched between 2 (data phase) and 1 (scan
    // phase). Scan path's encoded side-set values use only the LSB
    // (CLK), so reducing the count to 1 is equivalent to 1-bit
    // side-set.
    ...
    // ─── DATA path (PC 0) — 2-bit side-set, LAT via side-set ─────────
    a.bind(&mut data_cmd);
    a.out_with_side_set(pio::OutDestination::X, 15, 0b00);   // n_pre-1
    a.out_with_side_set(pio::OutDestination::Y, 16, 0b00);   // n_lat-1
    a.out_with_side_set(pio::OutDestination::NULL, 1, 0b00); // pad

    a.bind(&mut pre_loop);
    a.out_with_side_set(pio::OutDestination::PINS, 6, 0b00);
    a.out_with_side_set(pio::OutDestination::NULL, 2, 0b01); // CLK rise
    a.jmp_with_side_set(pio::JmpCondition::XDecNonZero, &mut pre_loop, 0b00);

    a.bind(&mut lat_loop);
    a.out_with_side_set(pio::OutDestination::PINS, 6, 0b10);  // LAT=1
    a.out_with_side_set(pio::OutDestination::NULL, 2, 0b11);  // CLK rise, LAT=1
    a.jmp_with_side_set(pio::JmpCondition::YDecNonZero, &mut lat_loop, 0b10);
    a.bind(&mut data_wrap_source);

    // ─── SCAN path ──────────────────────────────────────────────────
    // Side-set values only use LSB (upper bit ignored when
    // PINCTRL.SIDESET_COUNT=1 at runtime).
    ...
    a.set_with_side_set(pio::SetDestination::PINS, 1, 0b00);   // OE on
    ...
    a.set_with_side_set(pio::SetDestination::PINS, 0, 0b00);   // OE off
    a.out_with_side_set(pio::OutDestination::NULL, 5, 0b00);   // drain
    ...
    // Force install at PIO offset 0 so our labels stay within 5-bit
    // PC range. Default install picks highest free slot, which with
    // 25 instructions lands us at offset 7 → labels overflow 31.
    ...
    // ── 3b. Compute phase-swap values ───────────────────────────────
    // Label positions in the program (must stay in sync with the
    // assembly above):
    //   data_cmd         = 0
    //   data_wrap_source = 9   (after 9 data instructions: header×3 +
    //                           pre_loop×3 + lat_loop×3)
    //   scan_entry       = 9   (same slot as data_wrap_source — they're
    //                           consecutive binds; scan_entry is the
    //                           address of the first SCAN instruction,
    //                           never reached via natural advance from
    //                           data path because data's wrap intercepts)
    //   scan_wrap_source = 25  (after 16 more scan instructions)
    const DATA_CMD_LOCAL: u32 = 0;
    const DATA_WRAP_SRC_LOCAL: u32 = 9;
    const SCAN_ENTRY_LOCAL: u32 = 9;
    const SCAN_WRAP_SRC_LOCAL: u32 = 25;

    let data_cmd_pc = prog_offset + DATA_CMD_LOCAL;
    let scan_entry_pc = prog_offset + SCAN_ENTRY_LOCAL;

    // Read PINCTRL and EXECCTRL values configured by PIOBuilder
    // (assemble_with_wrap installed data's wrap, side_set_count=2).
    ...
    // PINCTRL bits: [31:29] SIDESET_COUNT. Data=2, Scan=1.
    const PINCTRL_SIDESET_COUNT_MASK: u32 = 0x7 << 29;
    let pinctrl_no_ss = base_pinctrl & !PINCTRL_SIDESET_COUNT_MASK;
    let data_pinctrl = pinctrl_no_ss | (2 << 29);
    let scan_pinctrl = pinctrl_no_ss | (1 << 29);

    // EXECCTRL bits: [16:12] WRAP_TOP, [11:7] WRAP_BOTTOM.
    ...
    let data_execctrl = execctrl_no_wrap
        | (((prog_offset + DATA_WRAP_SRC_LOCAL) & 0x1F) << 12)
        | (((prog_offset + DATA_CMD_LOCAL) & 0x1F) << 7);
    let scan_execctrl = execctrl_no_wrap
        | (((prog_offset + SCAN_WRAP_SRC_LOCAL) & 0x1F) << 12)
        | (((prog_offset + SCAN_ENTRY_LOCAL) & 0x1F) << 7);

    defmt::info!(
        "spwm_26 PIO: data_cmd={=u32} scan_entry={=u32}  data_wrap={=u32}..{=u32}  scan_wrap={=u32}..{=u32}",
        data_cmd_pc, scan_entry_pc,
        prog_offset + DATA_CMD_LOCAL, prog_offset + DATA_WRAP_SRC_LOCAL,
        prog_offset + SCAN_ENTRY_LOCAL, prog_offset + SCAN_WRAP_SRC_LOCAL,
    );

    // Phase-swap closures. Call after `wait_txstall` (SM is stalled,
    // SMx_INSTR writes apply immediately to the stalled PC).
    ...
    defmt::info!("spwm_26: unified program + persistent PINCTRL/EXECCTRL swap, RGB=1");
    ...
    loop {
        ...
        // SM stays enabled throughout the frame; swap_to_data/scan do
        // the persistent EXECCTRL/PINCTRL/INSTR writes at boundaries.
        set_clkdiv(2);
        swap_to_data();
