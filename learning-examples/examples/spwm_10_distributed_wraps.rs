//! # DP3364S S-PWM — Step 10: distributed wraps (E16)
//!
//! Tenth in the SPWM progression. Copied from spwm_9 to test E16, the
//! first candidate echo mitigation identified by the spwm_9 result
//! (adjacency theory confirmed, echo integrates over per-frame wraps).
//!
//! > **Hypothesis.** If echo intensity integrates approximately
//! > linearly over per-frame wrap events (spwm_9 showed 48-wraps-
//! > visible vs. 1-wrap-invisible), then spreading the 48 intra-frame
//! > wraps across all scan_lines should drop echo-per-row below the
//! > visibility threshold.
//!
//! Instead of rotating every cycle by a fixed offset, rotate each
//! cycle by a *different* offset so each wrap hits a different
//! destination scan_line:
//!
//! ```text
//!   rotate_for_cycle(N)        = N mod SCAN_LINES   for N < last
//!   rotate_for_cycle(last)     = 0                  // unrotated
//! ```
//!
//! The last post-scan cycle stays unrotated `[0..31]` so the last
//! scan_line before VSYNC is 31 (required for clean write-pointer
//! reset — E1 null in spwm_8).
//!
//! ## Wrap distribution (per frame)
//!
//! A wrap happens at every cycle boundary. For cycle N < 49:
//! `last_emitted(N) = (rotate[N] - 1) mod 32` and
//! `first_emitted(N+1) = rotate[N+1]`. Plus the frame boundary
//! `last_emitted(49)=31 → first_emitted(0 of next frame)=0`.
//! With `rotate[N]=N mod 32` (N<49) and `rotate[49]=0`:
//!
//! | Dest row | Wrap count | Source rows                             |
//! |----------|------------|------------------------------------------|
//! |    0     |      3     | 30 (N=31), 15 (N=48), **31** (frame)     |
//! |    1     |      2     | **31** (N=0), **31** (N=32)              |
//! |    2     |      2     | 0 (N=1), 0 (N=33)                        |
//! |   ...    |      2     | shifts by 1 each row; see code           |
//! |   16     |      2     | 14 (N=15), 14 (N=47)                     |
//! |  17..31  |      1     | 15, 16, …, 29                            |
//!
//! vs. spwm_9's 48 wraps concentrated on one row. Worst-hit dest row
//! in E16 sees 3/48 ≈ 6% of spwm_9's echo intensity; typical row
//! sees 2/48 ≈ 4%. If that's below threshold, echo is effectively
//! gone.
//!
//! ## Test pattern
//!
//! Colour scan_line 31 bright: row 31 cyan (upper), row 63 yellow
//! (lower). scan_line 31 is the wrap *source* for 3 intra-frame
//! wraps: two into dest row 1 / 33, and one into dest row 0 / 32
//! (frame boundary).
//!
//! Predicted visible echo locations:
//! - row 0 / 32: 1× echo (cyan / yellow)
//! - row 1 / 33: 2× echo (cyan / yellow)
//! - everywhere else: black (those dest rows get wraps from other
//!   source rows, which we keep black here)
//!
//! ## Caveats
//!
//! - **Write-pointer shift confound (inherited from spwm_9).** E15
//!   showed that rotation perturbs the VSYNC reset even when the
//!   last cycle is unrotated (image shifted +16 rows under
//!   ROTATE_OFFSET=15). Per-cycle varying rotation may shift the
//!   image further or differently. Treat any displacement of the
//!   bright source rows as a confound, not part of the echo finding.
//!
//! ```sh
//! cargo run --release --example spwm_10_distributed_wraps
//! ```
//!
//! ## Success criterion
//!
//! Echoes on rows {0, 1, 32, 33} at or below visibility threshold.
//!   - If invisible → continue to E17 (POST_SCAN_CYCLES sweep) to
//!     characterise the integration curve.
//!   - If still visible → note residual vs. spwm_9, move to E19
//!     (multi-DMA per frame).
...
// E16 (this experiment): per-cycle rotation. Last cycle stays
// unrotated so the final scan before VSYNC is scan_line 31.
const fn rotate_for_cycle(cycle: usize) -> usize {
    if cycle == POST_SCAN_CYCLES - 1 { 0 } else { cycle % SCAN_LINES }
}
...
fn init_frame_headers(buf: &mut [u32; FRAME_WORDS]) {
    ...
    for cycle in 0..POST_SCAN_CYCLES {
        let rotate = rotate_for_cycle(cycle);
...
/// Echo-diagnostic pattern for E16. All black except scan_line 31:
///   - row 31 bright cyan   (upper — wrap source for 3 intra-frame
///                            wraps into rows 0, 1, 1)
///   - row 63 bright yellow (lower — same for rows 32, 33, 33)
///
/// Predicted echoes: 1× on rows 0 / 32, 2× on rows 1 / 33. All other
/// non-source rows should remain black.
fn fill_pixels(_offset: u8) {
    let pixels = unsafe { &mut *core::ptr::addr_of_mut!(PIXELS) };
    for row in 0..64usize {
        let c = if row == 31 {
            Rgb::new(0, 255, 255)     // upper wrap-31 source — cyan
        } else if row == 63 {
            Rgb::new(255, 255, 0)     // lower wrap-31 source — yellow
        } else {
            Rgb::BLACK
        };
